...
const TransactionHistoryModal: React.FC<{ onClose: () => void }> = ({
  ...
  const [filter, setFilter] = useState("All");
  const [transactions, setTransactions] = useState<Transaction[]>([]); // Explicit type for transactions
  const [isLoading, setIsLoading] = useState(true);
  ...
  const getTransactionDetails = useCallback(async () => {
    try {
      setIsLoading(true);
      const { data: responseData } = await getData("/wallet/transactions");
      setTransactions(responseData.transactions); // Assuming responseData.transactions matches the Transaction interface
      setError(null);
    } catch (error: any) {
      setError(
        error.message || "An error occurred while fetching transactions"
